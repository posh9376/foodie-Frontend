...
const Navbar = ({ isLoggedIn, setIsLoggedIn }) => {
  ...
  return (
    <nav className="navbar">
      <Link to="/" className="navbar-logo">RECIPE FINDER</Link>
      ...
      <div className={`navbar-links ${isMobileMenuOpen ? 'mobile-visible' : 'mobile-hidden'}`}>
        <Link to="/" className="navbar-link">Home</Link>
        {!isLoggedIn && <Link to="/register" className="navbar-link">Register</Link>}
        {isLoggedIn ? (
          <>
            <button className="navbar-button" onClick={handleLogout}>Logout</button>
            <Link to="/addrecipes" className="navbar-link">Add Recipes</Link>
          </>
        ) : (
